...
function Products() {
  ...
  // Estado inicial basado en parámetros de URL
  ...
  return (
    <div className="min-h-screen bg-[#020209] text-white py-12 px-4 sm:px-6 lg:px-8">
      ...
      {/* Carrito flotante */}
      {showCart && (
  <div className="fixed bottom-4 right-4 left-4 sm:left-auto bg-[#0f0f1a] rounded-xl shadow-2xl border border-[#F348B7] z-40 w-auto sm:w-80">
    <div className="p-4">
      ...
      {/* Contenedor del scroll con estilos personalizados */}
      <div className="max-h-64 overflow-y-auto mb-4 scrollbar-thin scrollbar-thumb-[#F348B7] scrollbar-track-[#1a1a2e] scrollbar-rounded">
        {cart.length === 0 ? (
          <p className="font-poppins text-sm text-gray-400">No hay productos</p>
        ) : (
          <ul className="space-y-3 pr-2"> {/* Añadido padding-right para evitar que el contenido toque el scrollbar */}
            {cart.map(item => (
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {/* Filtros y búsqueda */}
        <div className="mb-8 sm:mb-12 bg-[#0f0f1a] p-4 sm:p-6 rounded-xl shadow-lg">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
              {/* Barra de búsqueda */}
              <div className="md:col-span-2 lg:col-span-1">
              <label htmlFor="search" className="block font-poppins font-medium mb-2 text-sm sm:text-base">
                  Buscar productos
                </label>
                <input
                  type="text"
                  ...
                  className="w-full px-3 sm:px-4 py-2 text-sm sm:text-base rounded-lg bg-[#1a1a2e] border border-[#2d2d42] focus:border-[#F348B7] focus:ring-1 focus:ring-[#F348B7] transition-all"
                  value={searchTerm}
  ...
  {filteredProducts.length > 0 ? (
    filteredProducts.map((product) => (
      <div key={product.id} className="bg-[#1a1a2e] p-4 rounded-lg border border-[#2d2d42]">
        {/* Primera fila: Imagen + Nombre + Precio */}
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="font-jost font-bold text-white text-base truncate">
              {product.name}
            </h3>
            <div className="flex justify-between items-center mt-1">
              ...
              <span className="font-jost font-bold text-[#F348B7] text-sm whitespace-nowrap ml-2">
        ...
        <p className="font-poppins text-sm text-gray-300 mt-3 line-clamp-2">
          {product.description}
        </p>

        {/* Tercera fila: Botón (estilo consistente con desktop) */}
        <button 
          onClick={() => handleOrderClick(product)}
          className="w-full mt-3 px-4 py-2 text-sm font-jost font-bold text-white 
          border-2 border-[#F348B7] rounded-xl hover:bg-[#F348B7] 
          transition-all duration-300 flex items-center justify-center gap-2"
        >
          <span>Pedir despacho</span>
          <span className="text-xs">GRATIS</span>
        </button>
      </div>
    ))
  ) : (
    <div className="text-center font-poppins text-gray-400 py-8">
  ...
  <table className="hidden md:table min-w-full divide-y divide-[#2d2d42]">
    <thead className="bg-[#1a1a2e]">
      <tr>
        <th scope="col" className="px-4 py-3 lg:px-6 lg:py-4 text-left font-jost font-bold text-[#B3F1FE] uppercase tracking-wider text-sm lg:text-base">
          Producto
        </th>
        <th scope="col" className="px-4 py-3 lg:px-6 lg:py-4 text-left font-jost font-bold text-[#B3F1FE] uppercase tracking-wider text-sm lg:text-base">
          Descripción
    ...
    <tbody className="bg-[#0f0f1a] divide-y divide-[#2d2d42]">
      {filteredProducts.length > 0 ? (
            ...
            <td className="px-4 py-3 lg:px-6 lg:py-4 whitespace-nowrap">
              <div className="flex items-center">
                <div className="flex-shrink-0 h-12 w-12 lg:h-16 lg:w-16">
                  <img className="h-12 w-12 lg:h-16 lg:w-16 rounded-lg object-cover" src={product.image} alt={product.name} />
                </div>
                <div className="ml-3 lg:ml-4">
                  <div className="font-jost font-bold text-white text-sm lg:text-base">{product.name}</div>
                  <div className="font-poppins text-xs lg:text-sm text-gray-300 capitalize">{product.category}</div>
                </div>
              </div>
            </td>
            <td className="px-4 py-3 lg:px-6 lg:py-4">
              <div className="font-poppins text-gray-300 text-sm lg:text-base line-clamp-2">
                {product.description}
              </div>
            </td>
            <td className="px-4 py-3 lg:px-6 lg:py-4 whitespace-nowrap">
              <div className="font-poppins text-gray-300 text-sm lg:text-base capitalize">
                {product.subcategory}
              </div>
            </td>
            <td className="px-4 py-3 lg:px-6 lg:py-4 whitespace-nowrap">
              <div className="font-jost font-bold text-[#F348B7] text-sm lg:text-base">
                {formatPrice(product.price)}
              </div>
            </td>
            <td className="px-4 py-3 lg:px-6 lg:py-4 whitespace-nowrap">
              <button 
                onClick={() => handleOrderClick(product)}
                className="inline-block px-3 py-1 lg:px-6 lg:py-2 font-jost font-bold text-white 
                border-2 border-[#F348B7] rounded-xl hover:bg-[#F348B7] 
        ...
        <div className="mt-8 sm:mt-12 text-center">
          <Link
            to="/"
            className="inline-block px-6 py-2 sm:px-8 sm:py-3 font-jost font-bold text-white border-2 border-[#B3F1FE] rounded-xl 
            hover:bg-[#B3F1FE] hover:text-black transition-all duration-300 text-sm sm:text-base"
          >
            Volver al inicio
          </Link>
        </div>
